...
function Dates({
  ...
  function checkIfCurrentMonth(index) {
    const date = new Date(currentYear, Number(currentMonth) - 1, currentWeek[0]);
  ...
  return (
    <DatesWrapper>
      ...
      <MonthContainer>
        <ArrowBackIosIcon className='arrow-back' onClick={() => changeWeek('back')} />
        <span>{monthMap[currentMonth]} {currentYear}</span>
        <ArrowForwardIosIcon className='arrow-forward' onClick={() => changeWeek('forward')} />
      </MonthContainer>
    </DatesWrapper>
  );
}
